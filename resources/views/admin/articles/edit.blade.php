...
<div class="flex justify-center items-center flex-col mt-8 mb-12">
	...
	<form method="POST" action="{{ route('articles.update', $article) }}" enctype="multipart/form-data" class="w-2/4 bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
		@csrf
		@method('PATCH')
		<div class="mb-4">
				<label for="title" class="block mb-1">Title</label>
				<input type="text" name="title" id="title" class="w-full px-4 py-2 border rounded-md focus:outline-none focus:border-blue-500" value="{{ old('title', $article->title) }}" required>
		</div>
		...
		<div class="mb-4">
			...
			<select name="category_id" id="category_id" class="w-full px-4 py-2 border rounded-md focus:outline-none focus:border-blue-500">
				<option value="{{ old('category', $article->category) }}" class="text-gray-500"></option>
		...
		<div class="mb-4">
			...
			<textarea name="description" id="description" class="w-full px-4 py-2 border rounded-md focus:outline-none focus:border-blue-500" >{{ old('description', $article->description) }} </textarea>
		</div>
